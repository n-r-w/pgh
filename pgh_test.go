...
func Test_InsertSplit_SendBatch(t *testing.T) {
	...
	for i := range rowsCount {
		rows = append(rows, Args{i, fmt.Sprintf("test %d", i)})
	}
...
func Test_InsertSplitQuery(t *testing.T) {
	...
	insertSQL := Builder().Insert("test_table").Columns("name", "age").Suffix("RETURNING id")
	var actualIDs []int64
...
func Test_ExecSplit(t *testing.T) {
	...
	for i := range queries {
		queriesSQ = append(queriesSQ, Builder().Select("*").From("test_table").Where(sq.Eq{"id": i}))
	}
...
func TestExecBatch(t *testing.T) {
	...
	queries := []sq.Sqlizer{
		Builder().Insert("users").Columns("name", "age").Values("John Doe", 30),
		Builder().Insert("users").Columns("name", "age").Values("Jane Smith", 25),
		Builder().Insert("users").Columns("name", "age").Values("Bob", 20),
	}
